# Lambda zips MUST be built by `npm run build` from the repo root before
# running `terraform apply`. The build script produces one zip per handler
# under `build/<name>.zip`. If a zip is missing, `terraform plan` will fail.
