...
#
# Shared policy document. Used by the ECS task role below; the legacy
# Lambda execution roles were removed when lambda.tf was decommissioned.
...
# ---- Secrets Manager read (Turnstile + ElevenLabs) ----
...
#   - ecs_task: the running container's identity. Holds the full set of
#     runtime permissions — Bedrock + DynamoDB + Secrets Manager +
#     S3 finetune-traces put.
#
...
# wired to detach on cost overrun (see budgets.tf).
